...
description: >
  ...
  ratings and specific remediation guidance. Automatically forks the
  ai-generated-code companion skill when reviewing AI-assisted pull requests.
...
## Step 9: AI-Generated Code Patterns (Forked Companion Skill)

If the pull request or codebase shows any indicators of AI-generated code, fork and apply the `ai-generated-code` companion skill in parallel with the remaining steps of this review:

```
Read: skills/appsec/ai-generated-code/SKILL.md
```

**AI-generation indicators (any one is sufficient to trigger):**
- Commit message references an AI tool (`co-authored-by: GitHub Copilot`, `generated by Claude`, etc.)
- PR body or labels contain `ai-generated`, `copilot`, `cursor`, or similar
- Files contain placeholder strings: `changeme`, `your_secret_here`, `replace_me`, `TODO: change`
- Entire files added in a single commit with no prior history

**If no indicators are present:** Run the companion skill anyway on the highest-risk files (authentication handlers, data-access layers, API controllers). AI patterns appear in human-reviewed code that incorporated AI suggestions without disclosure.

The `ai-generated-code` skill runs as `context: fork` — it executes as an isolated subagent and returns a **AI-Generated Code Patterns** section to merge into this report's findings.
