...
## Appendix A: AI-Generated Code Review

> **Context:** Empirical research (grith.ai, 2025) found that 87% of AI-generated pull requests ship at least one security vulnerability. LLMs trained on public code have learned — and readily reproduce — a set of well-known anti-patterns. This appendix surfaces the most prevalent ones, provides detection rationale, and supplies ready-to-use Semgrep rule stubs for CI integration.

---

### A.1 f-String SQL Injection in AI-Suggested Query Builders

**CWE:** CWE-89 (SQL Injection)
**ASVS Control:** V5.3.4

**Why AI generates it:** LLMs trained on Python tutorials and Stack Overflow answers frequently produce f-string query construction because it is the simplest, most readable pattern. The model has seen this pattern countless times in non-security-critical examples and reproduces it faithfully.

**Vulnerable pattern (Python):**
```python
# AI-generated: "build a query that fetches users by name"
...
**Also watch for AI-generated query builders:**
```python
# AI-generated ORM bypass -- same root cause, less obvious
...
**Remediation:**
```python
# Parameterized -- safe regardless of user input
...
**Review checklist:**
- [ ] Grep for `cursor.execute(f"` and `cursor.execute("` + concatenation across the entire codebase, not just the diff.
- [ ] Verify ORM query builders do not fall back to raw SQL for dynamic column/table names.
- [ ] Confirm table/column names from user input are validated against a strict allowlist.

---

### A.2 Missing Auth Decorators on AI-Generated REST Controller Methods

**CWE:** CWE-306 (Missing Authentication for Critical Function)
**ASVS Control:** V4.1.1

**Why AI generates it:** When prompted to "add an endpoint that does X," LLMs focus on the functional path and omit security middleware. Auth decorators or middleware are not part of the functional description, so the model leaves them out — especially in iterative sessions where the user asks for one method at a time.

**Vulnerable pattern (Python/Flask):**
```python
# AI-generated: "add an endpoint to delete a user by ID"
@app.route('/api/admin/users/<int:user_id>', methods=['DELETE'])
def delete_user(user_id):           # ← no @login_required, no role check
...
```javascript
// AI-generated: "add a route to export all orders as CSV"
app.get('/api/export/orders', async (req, res) => {   // ← no auth middleware
...
```
```javascript
app.get('/api/export/orders', requireAuth, requireRole('admin'), async (req, res) => {
  ...
});
```
...
rules:
    ...
    patterns:
      ...
      - pattern-not: |
          ...
          def $FUNC(...):
              ...
    message: >
      Flask route handler '$FUNC' has no authentication decorator.
      Confirm this endpoint is intentionally public, or apply @login_required
      / @jwt_required before merge.
    languages: [python]
    severity: WARNING
    metadata:
      cwe: CWE-306
  ...
  - id: ai-gen-missing-auth-express
    ...
    message: >
      Express route registered without middleware. Verify authentication
      and authorization are applied, especially for data-mutating or
...
**Review checklist:**
- [ ] Enumerate every route registration and confirm each has an explicit auth middleware or is documented as intentionally public.
- [ ] Pay extra attention to admin, export, delete, and bulk-operation endpoints — these are the ones AI most often forgets to protect.
- [ ] Confirm that auth middleware is applied at the router level and not only on some routes via per-handler decorators.

---

### A.3 Hardcoded Test Credentials Left in Production by AI Code Generators

**CWE:** CWE-798 (Use of Hard-coded Credentials)
**ASVS Control:** V2.10.1

**Why AI generates it:** LLMs generate self-contained, immediately runnable examples. To make the example work without setup, they embed credentials inline. When developers copy AI output into production files without cleanup — or when AI generates scaffolding directly into non-test files — those credentials ship.

**Vulnerable patterns (multiple languages):**
```python
# AI-generated: "connect to the database"
DB_CONFIG = {
    "host": "localhost",
    "user": "admin",
    "password": "Test1234!",      # ← AI placeholder credential
    "database": "myapp"
}
```
```javascript
// AI-generated: "add a JWT signing secret"
const JWT_SECRET = "supersecretkey123";   // ← hardcoded
```
```yaml
# AI-generated: docker-compose or k8s manifest
environment:
  - POSTGRES_PASSWORD=changeme      # ← AI default
  - REDIS_AUTH=test_password
```
```java
// AI-generated: "connect to the test database"
private static final String PASSWORD = "admin123";  // ← left in prod class
```

**Common AI credential fingerprints to grep for:**
- `Test1234`, `changeme`, `password123`, `secret123`, `supersecret`
- `your_secret_here`, `replace_me`, `TODO: change`
...
rules:
  - id: ai-gen-hardcoded-test-credentials
    ...
    message: >
      ...
      or secrets manager reference before merge.
  ...
  - id: ai-gen-hardcoded-credential-high-entropy
    ...
    metavariable-regex:
      metavariable: $KEY
      regex: (?i)(password|passwd|secret|token|api_key|auth_key|jwt_secret|signing_key)
    metavariable-regex:
      metavariable: $VALUE
      regex: .{8,}   # any non-trivial string value
    message: >
      Variable with a credential-like name '$KEY' is assigned a string
      literal. Confirm this is not a hardcoded secret — use environment
...
**Review checklist:**
- [ ] Scan all non-test files for known AI placeholder strings (see fingerprints list above).
- [ ] Run `git log --all -p | grep -iE "(password|secret|token)\s*=" | grep -v "os\.environ"` to catch credentials already committed.
- [ ] Verify CI/CD pipelines do not inject plaintext secrets via environment variable files committed to the repo.
- [ ] Ensure a pre-commit hook (gitleaks, detect-secrets) is active to catch future occurrences.

---

### A.4 "Safe Eval" Library False Safety (SimpleEval / RestrictedPython Bypasses)

**CWE:** CWE-94 (Code Injection), CWE-78 (OS Command Injection)
**ASVS Control:** V5.2.4
**Active CVE:** CVE-2026-32640 — SimpleEval < 1.0.5, CVSS 8.7. Attacker-controlled expressions bypass the sandbox via dunder attribute access, enabling arbitrary code execution.

**Why AI generates it:** LLMs are aware that `eval()` is dangerous and will often "improve" their own suggestions by substituting `simpleeval` or `RestrictedPython` — libraries marketed as safe alternatives. The model does not know about the full bypass surface and presents these as secure solutions. Developers accept the suggestion without auditing the library's CVE history.

**Vulnerable pattern (Python — SimpleEval):**
```python
# AI-generated: "safely evaluate a user-supplied math expression"
from simpleeval import simple_eval

def calculate(expression: str) -> float:
    return simple_eval(expression)   # ← not safe for untrusted input
```

**Bypass example (SimpleEval < 1.0.5, CVE-2026-32640):**
```python
# Attacker payload — bypasses SimpleEval sandbox via dunder chain
payload = "__import__('os').system('id')"
simple_eval(payload)   # executes arbitrary shell command
```

**Vulnerable pattern (Python — RestrictedPython):**
```python
# AI-generated: "run user-provided Python snippets safely"
from RestrictedPython import compile_restricted, safe_globals

def run_user_code(code: str):
    exec(compile_restricted(code, '<string>', 'exec'), safe_globals)
    # RestrictedPython bypasses are documented in multiple public PoCs
```

**Remediation options (in order of preference):**

1. **Avoid eval entirely.** Parse the expression with a grammar (`pyparsing`, `lark`) and evaluate only AST nodes you explicitly whitelist.
2. **If expression evaluation is required**, use `ast.literal_eval` for data-only expressions (no function calls). For arithmetic, walk the AST manually.
3. **Sandboxed subprocess.** Run untrusted code in an isolated subprocess with no network access, a read-only filesystem, and a resource limit (seccomp, cgroups). Never in the main process.
4. **Upgrade SimpleEval** to ≥ 1.0.5 immediately if it is already deployed, and limit the names/functions available in the evaluation context.

```python
# Safe alternative: AST-based arithmetic only
import ast, operator
...
rules:
  - id: ai-gen-simpleeval-unsafe
    ...
    message: >
      SimpleEval is not a safe sandbox for untrusted input. CVE-2026-32640
      demonstrates code execution via dunder attribute chains in versions
      < 1.0.5. Use AST-based parsing with an explicit node allowlist instead.
    languages: [python]
    severity: ERROR
    metadata:
      cwe: CWE-94
  ...
  - id: ai-gen-restrictedpython-unsafe
    ...
    message: >
      RestrictedPython has known sandbox escape techniques documented in
      public PoCs. Do not use it to execute untrusted user-supplied code.
      Prefer AST-level parsing with an explicit operator/literal allowlist.
    languages: [python]
    severity: ERROR
    metadata:
      cwe: CWE-94
  ...
  - id: ai-gen-eval-any
    pattern: eval($EXPR)
    message: >
      Direct eval() call detected. If $EXPR contains any user-controlled
      input, this is a critical code injection vulnerability. Consider
      ast.literal_eval() for data, or a purpose-built expression parser.
...
**Review checklist:**
- [ ] Search the entire codebase for `simpleeval`, `RestrictedPython`, `eval(`, `exec(` — flag every occurrence.
- [ ] For any `simpleeval` usage, confirm the version is ≥ 1.0.5 and that the `names` and `functions` arguments restrict the evaluation context to safe operations only.
- [ ] Verify no user-controlled string reaches an `eval`, `exec`, `compile`, or expression-evaluation function through any code path.
- [ ] If expression evaluation is a product requirement, require a security design review before merging AI-generated implementations.

---

### Appendix A: AI-Pattern Coverage Summary

| Pattern | CWE | ASVS | Severity | Semgrep Rule ID |
|---|---|---|---|---|
| f-String SQL injection | CWE-89 | V5.3.4 | Critical | `ai-gen-fstring-sql-injection` |
| Missing auth decorator | CWE-306 | V4.1.1 | High | `ai-gen-missing-auth-decorator-flask`, `ai-gen-missing-auth-express` |
| Hardcoded test credentials | CWE-798 | V2.10.1 | High | `ai-gen-hardcoded-test-credentials`, `ai-gen-hardcoded-credential-high-entropy` |
| Safe-eval library bypass | CWE-94 | V5.2.4 | Critical | `ai-gen-simpleeval-unsafe`, `ai-gen-restrictedpython-unsafe`, `ai-gen-eval-any` |

> **Integration note:** All Semgrep rule stubs above should be placed in `.semgrep/ai-generated-patterns.yml` in the target repository and run as a dedicated CI job on every PR touching AI-assisted commits (identifiable via commit message conventions or PR labels).
